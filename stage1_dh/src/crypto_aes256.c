#include "crypto_aes256.h"

#include <string.h>

static const uint8_t sbox[256] = {
	,
};

static uint32_t be32_load(const uint8_t b[4]) {
	return ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) | ((uint32_t)b[2] << 8) | (uint32_t)b[3];
}

static uint8_t xtime(uint8_t a) {
	return (uint8_t)((a << 1) ^ ((a & 0x80) ? 0x1b : 0x00));
}

static void sub_bytes(uint8_t s[16]) {
	for (int i = 0; i < 16; i++) {
		s[i] = sbox[s[i]];
	}
}

static void shift_rows(uint8_t s[16]) {
	uint8_t t[16];
	memcpy(t, s, 16);

	s[0] = t[0];
	s[4] = t[4];
	s[8] = t[8];
	s[12] = t[12];

	s[1] = t[5];
	s[5] = t[9];
	s[9] = t[13];
	s[13] = t[1];

	s[2] = t[10];
	s[6] = t[14];
	s[10] = t[2];
	s[14] = t[6];

	s[3] = t[15];
	s[7] = t[3];
	s[11] = t[7];
	s[15] = t[11];
}

static void mix_columns(uint8_t s[16]) {
	for (int c = 0; c < 4; c++) {
		uint8_t a0 = s[c * 4 + 0];
		uint8_t a1 = s[c * 4 + 1];
		uint8_t a2 = s[c * 4 + 2];
		uint8_t a3 = s[c * 4 + 3];

		uint8_t r0 = (uint8_t)(xtime(a0) ^ (uint8_t)(xtime(a1) ^ a1) ^ a2 ^ a3);
		uint8_t r1 = (uint8_t)(a0 ^ xtime(a1) ^ (uint8_t)(xtime(a2) ^ a2) ^ a3);
		uint8_t r2 = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (uint8_t)(xtime(a3) ^ a3));
		uint8_t r3 = (uint8_t)((uint8_t)(xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3));

		s[c * 4 + 0] = r0;
		s[c * 4 + 1] = r1;
		s[c * 4 + 2] = r2;
		s[c * 4 + 3] = r3;
	}
}

static void add_round_key(uint8_t s[16], const uint32_t rk[4]) {
	for (int i = 0; i < 4; i++) {
		uint32_t w = rk[i];
		s[i * 4 + 0] ^= (uint8_t)(w >> 24);
		s[i * 4 + 1] ^= (uint8_t)(w >> 16);
		s[i * 4 + 2] ^= (uint8_t)(w >> 8);
		s[i * 4 + 3] ^= (uint8_t)w;
	}
}

static uint32_t subword(uint32_t w) {
	uint8_t b0 = sbox[(w >> 24) & 0xff];
	uint8_t b1 = sbox[(w >> 16) & 0xff];
	uint8_t b2 = sbox[(w >> 8) & 0xff];
	uint8_t b3 = sbox[w & 0xff];
	return ((uint32_t)b0 << 24) | ((uint32_t)b1 << 16) | ((uint32_t)b2 << 8) | (uint32_t)b3;
}

static uint32_t rotword(uint32_t w) {
	return (w << 8) | (w >> 24);
}

void crypto_aes256_init(struct crypto_aes256_ctx *ctx, const uint8_t key[32]) {
	static const uint32_t rcon[15] = {
		0x00000000U,
		0x01000000U,
		0x02000000U,
		0x04000000U,
		0x08000000U,
		0x10000000U,
		0x20000000U,
		0x40000000U,
		0x80000000U,
		0x1b000000U,
		0x36000000U,
		0x6c000000U,
		0xd8000000U,
		0xab000000U,
		0x4d000000U,
	};

	for (int i = 0; i < 8; i++) {
		ctx->rk[i] = be32_load(&key[i * 4]);
	}
	for (int i = 8; i < 60; i++) {
		uint32_t temp = ctx->rk[i - 1];
		if (i % 8 == 0) {
			temp = subword(rotword(temp)) ^ rcon[i / 8];
		} else if (i % 8 == 4) {
			temp = subword(temp);
		}
		ctx->rk[i] = ctx->rk[i - 8] ^ temp;
	}
}

void crypto_aes256_encrypt_block(const struct crypto_aes256_ctx *ctx, const uint8_t in[16], uint8_t out[16]) {
	uint8_t s[16];
	memcpy(s, in, 16);

	add_round_key(s, &ctx->rk[0]);

	for (int round = 1; round <= 13; round++) {
		sub_bytes(s);
		shift_rows(s);
		mix_columns(s);
		add_round_key(s, &ctx->rk[round * 4]);
	}

	sub_bytes(s);
	shift_rows(s);
	add_round_key(s, &ctx->rk[14 * 4]);

	memcpy(out, s, 16);
}
